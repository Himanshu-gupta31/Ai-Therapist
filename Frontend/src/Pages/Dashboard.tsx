...
export default function UnifiedDashboard() {
  ...
  useEffect(() => {
    const updateAllCoins = async () => {
      for (const habit of habits) {
        if (habit.lastCheckIn && habit.id) {
          await addCoins(habit.id); // Await to avoid race conditions
        }
      }
    };
  ...
  useEffect(() => {
    // Load quotes for all habits on mount
                ...
                return (
                  habit.id && (
                    <div key={habit.id} className="relative">
                      ...
                      {/* Centered View Calendar button at the bottom */}
            ...
            <div className="space-y-4">
              ...
              <div>
                ...
                <input
                  ...
                  placeholder="Enter your goal for this habit"
                  className="w-full px-3 py-2 bg-[#0d1117] border border-[#30363d] rounded-md focus:outline-none focus:ring-2 focus:ring-[#58a6ff] focus:border-transparent text-[#c9d1d9]"
                />
              </div>
              {/* Buttons */}
